...
def bls_search(return_type='short', **kwargs):
    ...
    - ``return_type='short'`` : The key in the dict will only contain the elements that
        ...
        this option will make it so each key is just {'state'=some_state}. If there are no iterable
        kwargs, it will behave like ``return_type='full'``.
    ...
    """
    def _single_series_search(data:str=None, state:str=None, msa:str=None, sa:bool=True):
        fips = _state_to_fips(state)
        geo = 'state' if fips!='00' else 'us'
        if geo == 'state':
            seas = 'S' if sa else 'U'
        if geo == 'us':
            seas = 'S1' if sa else 'U0'
        data_id = _find_data_series(data=data, geo=geo)
        return data_id.format(seas=seas, fips=fips)
